...
This document explains how to implement a `ReadOnlyDagView` adapter for DagIR. A `ReadOnlyDagView` is a lightweight, read-only, non-owning view over a foreign DAG. The view exposes a minimal API that the DagIR algorithms and IR builder use to traverse nodes and edges without copying the underlying graph.

See `include/dagir/ro_dag_view.hpp` for full concepts; this document provides a practical walkthrough and examples.

## Requirements (concept summary)

A type models `dagir::ReadOnlyDagView` when:

- It defines `using handle = ...;` and that type models `dagir::NodeHandle`.
- It provides `children(handle)` returning a range (input-range) of child edge-like elements; the value/reference either is a child handle or an `EdgeRef` (type providing `target() -> handle`).
- It provides `roots()` returning an input-range of `handle` values.

Optional:
- `start_guard(handle)` returning an RAII guard (for pinning resources during traversal). Adapters that don't require it can return `dagir::NoopGuard`.
...
struct IntDagView {
  ...
  auto roots() const { return std::views::iota(0ull, adjacency.size()) | std::views::transform([](auto i){ return IntHandle{i}; }); }
...
- `children` returns a range whose value_type is the `handle` type; that satisfies `dagir::ChildrenRange`.
- `roots()` returns a range of handles (we used `std::views` to create it lazily).

## Implementing an `EdgeRef`

If your backend needs edges that carry labels or other metadata, implement an `EdgeRef` type exposing `target()`.
...
If your backend requires pinning or a scoped lock during traversal, provide `start_guard(handle)` returning an RAII guard type. The `build_ir` implementation will call it when present.
...
Once you have a `ReadOnlyDagView`, you can write policy objects (labelers, attributors) that accept the view and handles. See `docs/IMPLEMENTING_POLICY.md` for examples that use `dagir::ir_attrs` keys.
...
With these in place you can use DagIR traversal algorithms and `build_ir` to emit an `IRGraph` for rendering or analysis.
