...
A `node_labeler` is any callable compatible with the `node_labeler` concept (see `include/dagir/concepts/read_only_dag_view.hpp`). The simplest form receives `(const View&, const Handle&)` and returns a `std::string`.
...
- `dagir::ir_attrs::k_label` — Node or edge label text. Backends render this as visible text.
- `dagir::ir_attrs::k_tooltip` — Short help text or title used as a tooltip.
- `dagir::ir_attrs::k_color` — Stroke/edge color or text color.
- `dagir::ir_attrs::k_fill` — Node fill color.
- `dagir::ir_attrs::k_style` — Styling hint (e.g., "dashed", "solid").
- `dagir::ir_attrs::k_shape` — Node shape hint (e.g., "box", "circle").
- `dagir::ir_attrs::k_pen` — Stroke width for edges or pen hint.
- `dagir::ir_attrs::k_font` — Font family name.
- `dagir::ir_attrs::k_font_size` — Font size hint.
- `dagir::ir_attrs::k_weight` — Weighting hint (layout influence).
- `dagir::ir_attrs::k_dir` — Edge direction hint (e.g., "forward", "both").
- `dagir::ir_attrs::k_id` — An explicit identifier (if backend needs it).
- `dagir::ir_attrs::k_width`, `dagir::ir_attrs::k_height` — Size hints for nodes.
- `dagir::ir_attrs::k_group` — Logical grouping name for layout or clustering.
- `dagir::ir_attrs::k_graph_label` — Graph-level label (use on `ir_graph::global_attrs`).

Notes:
- The keys are intentionally generic. Backends can map them to renderer-specific names.
- `ir_attrs` are `std::string_view` constants; `dagir::ir_attr` uses `std::string` for keys/values. Construct `ir_attr` from the constant when emitting (e.g., `dagir::ir_attr{std::string(dagir::ir_attrs::k_label), "value"}`).
...
`edge_attributor` callables are flexible. The `build_ir` helper in `include/dagir/build_ir.hpp` accepts various signatures (including forms that take `(view, parent, edge_like)` or `(parent, child)`), and the return type is interpreted as a `std::vector<dagir::ir_attr>`.
